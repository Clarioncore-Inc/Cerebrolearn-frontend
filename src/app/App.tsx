...
function AppContent() {
  ...
  useEffect(() => {
    if (currentPage === 'auth') {
      if (pageData?.postAuthRedirect) {
        setPostAuthRedirect(pageData.postAuthRedirect);
      }
      if (pageData?.authMode) {
        setAuthMode(pageData.authMode);
      }
    }
  ...
  useEffect(() => {
    ...
    if (user && currentPage === 'auth') {
      const pendingRedirect = postAuthRedirect ?? pageData?.postAuthRedirect;

      if (pendingRedirect?.page) {
        navigate(pageToPath(pendingRedirect.page), {
          replace: true,
          state: pendingRedirect.data || null,
        });
      ...
      currentPage !== 'iq-test-completion' &&
    ...
    switch (currentPage) {
      ...
      case 'creator-course-edit':
        return (
          <CourseCreatorDashboard
            onNavigate={handleNavigate}
            currentPage='creator-course-edit'
            initialSelectedCourse={pageData ?? null}
          />
        );
        ...
        return pageData ? (
          <LessonEditor
            ...
            onSave={(lesson) => {
              ...
              handleNavigate('creator-course-edit', pageData.course);
            }}
            onCancel={() =>
              handleNavigate('creator-course-edit', pageData.course)
            }
