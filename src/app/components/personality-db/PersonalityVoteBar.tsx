...
export function PersonalityVoteBar({ results, totalVotes, selectedType, onVote }: PersonalityVoteBarProps) {
  if (results.length === 0) {
    return (
      <p className="text-sm text-muted-foreground py-4 text-center">
        No votes yet for this system. Be the first to vote!
      </p>
    );
  }
    ...
    <div className="space-y-3">
      {results.map((result, index) => {
        ...
        return (
          <button
            key={result.typeCode}
            onClick={() => onVote(result.typeCode)}
            className={`w-full text-left group rounded-lg border p-3 transition-all ${
              isSelected
          ...
          >
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">{result.typeName}</span>
      ...
      <p className="text-xs text-muted-foreground text-center pt-2">
        {totalVotes.toLocaleString()} total votes · Click a type to cast your vote
      </p>
    </div>
  );
}
