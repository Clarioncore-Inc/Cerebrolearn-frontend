...
export function CollaborativePreview({
  courseData,
  onClose,
}: CollaborativePreviewProps) {
  ...
  const issues: PreviewIssue[] = [
    ...(courseData.sections
      ?.filter((s: any) => !s.title || s.lessons?.length === 0)
      .map((s: any, i: number) => ({
        id: `incomplete-${i}`,
    ...
    ...(!courseData.verificationRequests ||
    courseData.verificationRequests.length === 0
      ? [
          {
            id: 'unverified-1',
