        ...
        {content.videoUrl ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center text-white space-y-4">
              ...
              <div>
                <p className="text-xl font-semibold mb-2">{content.title || 'Video Lesson'}</p>
      ...
      <div className="prose prose-lg max-w-none dark:prose-invert">
        {content.html ? (
          ...
          <div className="space-y-4 text-muted-foreground leading-relaxed">
            ...
            {content.paragraphs && content.paragraphs.map((para: string, idx: number) => (
              <p key={idx}>{para}</p>
            ))}
          </div>
        )}
    ...
    <div className="space-y-6">
      <div className="relative">
        ...
        <div className="overflow-auto bg-muted/30 rounded-xl p-8 max-h-[600px]">
          <img
            src={content.imageUrl || 'https://images.unsplash.com/photo-1635070041078-e363dbe005cb?w=1200'}
              ...
              <div className="text-center space-y-4 max-w-2xl">
                ...
                {slides[currentSlide].bullets && (
                  <ul className="text-left space-y-2 list-disc list-inside">
                    {slides[currentSlide].bullets.map((bullet: string, idx: number) => (
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" variant="outline" onClick={handleCopy}>
              {copied ? <CheckCircle2 className="w-4 h-4 mr-2" /> : <FileCode className="w-4 h-4 mr-2" />}
...
// Main component
export function LessonPlayer({ lesson, course, onNavigate, onComplete }: LessonPlayerProps) {
  ...
  const loadProgress = async () => {
    try {
      const data = await apiCall(`/progress/${lesson.id}`);
      if (data.progress) {
        setProgress(data.progress.percent || 0);
        setCurrentStep(data.progress.state?.currentStep || 0);
      }
    } catch (error) {
  ...
  const handleQuizComplete = (score: number) => {
    ...
    toast.success(`Quiz completed! Score: ${score}% | +${Math.floor(score / 10)} XP`, {
      icon: <Trophy className="h-5 w-5 text-yellow-500" />,
    });
  };
      ...
      <div className="container max-w-5xl py-8 space-y-6">
        ...
        {showHint && currentContent?.hint && (
          <Card className="bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-800 border-2">
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground">{currentContent.hint}</p>
            </CardContent>
          </Card>
        )}
