      ...
      <div className="flex h-20 items-center justify-between md:px-8 lg:px-12 px-[24px] py-[0px]">
        <div className="flex items-center gap-12">
          ...
          >
            ...
            <span className="font-bold text-lg bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">CerebroLearn</span>
          </button>
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all inline-flex items-center gap-1 ${
                      ['catalog', 'course-notes', 'course-comparison', 'my-learning-path', 'bookmarks'].includes(currentPage)
                        ? 'bg-primary/10 text-primary'
                        : 'text-muted-foreground hover:text-foreground hover:bg-muted'
                    }`}
                  >
                ...
                <DropdownMenuContent align="start" className="w-56">
                  ...
                  <DropdownMenuItem onClick={() => onNavigate('my-learning-path')}>
                    <Target className="mr-2 h-4 w-4" />
                    My Learning Path
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onNavigate('course-comparison')}>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all inline-flex items-center gap-1 ${
                      ['progress-dashboard', 'learning-streak', 'leaderboard'].includes(currentPage)
                        ? 'bg-primary/10 text-primary'
                        : 'text-muted-foreground hover:text-foreground hover:bg-muted'
                    }`}
                  >
                ...
                <DropdownMenuContent align="start" className="w-56">
                  <DropdownMenuItem onClick={() => onNavigate('progress-dashboard')}>
                    <BarChart3 className="mr-2 h-4 w-4" />
                    Learning Dashboard
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onNavigate('learning-streak')}>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all inline-flex items-center gap-1 ${
                      ['iq-test-landing', 'browse-psychologists', 'student-sessions'].includes(currentPage)
                        ? 'bg-primary/10 text-primary'
                        : 'text-muted-foreground hover:text-foreground hover:bg-muted'
                    }`}
                  >
                ...
                <DropdownMenuContent align="start" className="w-56">
                  <DropdownMenuItem onClick={() => onNavigate('iq-test-landing')}>
                    <Brain className="mr-2 h-4 w-4" />
                    IQ Assessment
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onNavigate('browse-psychologists')}>
                    <Users className="mr-2 h-4 w-4" />
                    Find Psychologists
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onNavigate('student-sessions')}>
        ...
        <div className="flex items-center gap-6">
          ...
          {user && profile && (profile.role === 'admin' || profile.role === 'org_admin' || profile.role === 'instructor') && (
          ...
          {/* Navigation History */}
          {user && (
            <NavigationHistory currentPage={currentPage} onNavigate={onNavigate} />
          )}
            ...
            <div className="flex items-center gap-3">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  ...
                  {(profile.role === 'org_admin' || profile.role === 'admin') && (
                  ...
                  {profile.role === 'student' && (
                    <DropdownMenuItem onClick={() => onNavigate('student-payment-history')}>
                      <CreditCard className="mr-2 h-4 w-4" />
                      Payment History
                    </DropdownMenuItem>
                  )}
                  {profile.role === 'psychologist' && (
                    <DropdownMenuItem onClick={() => onNavigate('psychologist-earnings')}>
