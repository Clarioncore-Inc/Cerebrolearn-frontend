...
export function InstructorDashboard({ onNavigate }: InstructorDashboardProps) {
  ...
  const stats = [
    ...
    },
    {
      ...
      change: courses.length > 0 ? `${courses.filter(c => c.status === 'published').length}/${courses.length}` : '0/0',
      trend: 'neutral',
      icon: BookOpen,
  ...
  const recentActivity = [
    { type: 'enrollment', course: 'Introduction to Python', student: 'John Doe', time: '2 hours ago' },
    { type: 'review', course: 'Web Development Bootcamp', rating: 5, time: '5 hours ago' },
    { type: 'completion', course: 'React Masterclass', student: 'Jane Smith', time: '1 day ago' },
    { type: 'question', course: 'Data Science Fundamentals', student: 'Mike Johnson', time: '2 days ago' }
  ];
    ...
    <div className="min-h-screen bg-gradient-to-b from-background via-accent/30 to-background">
      <div className="py-6 px-4 md:py-6 md:px-6 lg:py-12 lg:px-12 space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-4xl font-bold mb-2 gradient-text">Instructor Dashboard</h1>
          ...
          <div className="flex gap-3">
            ...
            <Button onClick={() => setShowCreateWizard(true)} size="lg" className="bg-primary">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {stats.map((stat) => (
            <Card key={stat.title} className="relative overflow-hidden border-2 hover:border-primary/50 transition-all duration-300 shadow-none hover:shadow-md">
              <div className="absolute top-0 right-0 w-32 h-32 rounded-full blur-3xl opacity-20" style={{ background: stat.color.replace('text-', '') }}></div>
          ...
          <Card className="lg:col-span-2 border-2 shadow-none hover:shadow-md transition-all">
            <CardHeader className="border-b bg-accent/30">
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-2xl">Top Performing Courses</CardTitle>
              ...
              <div className="space-y-4">
                {topPerformingCourses.length > 0 ? (
                  topPerformingCourses.map((course, index) => (
                    <div key={course.id} className="flex items-center gap-4 p-4 rounded-xl bg-accent/50 hover:bg-accent transition-colors">
                ...
                {recentActivity.map((activity, index) => (
                  <div key={index} className="flex items-start gap-3 pb-4 border-b last:border-0 last:pb-0">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                      ...
                      activity.type === 'completion' ? 'bg-green-500/10 text-green-500' :
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate">{activity.course}</p>
                      <p className="text-xs text-muted-foreground">
                        {activity.type === 'enrollment' && `${activity.student} enrolled`}
                        {activity.type === 'review' && `New ${activity.rating}-star review`}
                        {activity.type === 'completion' && `${activity.student} completed`}
                        {activity.type === 'question' && `${activity.student} asked a question`}
                      </p>
          ...
          <CardHeader className="border-b bg-accent/30">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                ...
                <CardDescription>Manage and track all your courses</CardDescription>
              </div>
            ...
            <Tabs defaultValue="all" className="w-full">
              ...
              <TabsContent value="all">
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {courses.map((course) => (
                    <Card key={course.id} className="group overflow-hidden border-2 hover:border-primary/50 transition-all duration-300">
                        ...
                        <div className="absolute top-4 left-4 right-4 z-20 flex items-start justify-between">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => onNavigate('course', course)}>
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <div className="grid grid-cols-3 gap-3 text-center">
                          <div>
                            <div className="text-xl font-bold text-primary">{course.enrollments || 0}</div>
                            <div className="text-xs text-muted-foreground">Students</div>
                          </div>
                          <div>
                            <div className="text-xl font-bold text-primary">{course.rating || 4.5}</div>
                ...
                {courses.length === 0 && (
                  <Card className="p-12 text-center border-2 border-dashed">
                    ...
                    <p className="text-muted-foreground mb-6 max-w-md mx-auto">
                      Create your first course and start sharing your knowledge with students around the world
                    </p>
                    <Button size="lg" onClick={() => setShowCreateWizard(true)} className="bg-primary">
                      <Plus className="mr-2 h-5 w-5" />
                  ...
                  {courses.filter(c => c.status === 'published').map((course) => (
                    <Card key={course.id} className="border-2 hover:border-primary/50 transition-all">
                      <CardHeader>
                        ...
                        <CardDescription>{course.enrollments || 0} students enrolled</CardDescription>
                      </CardHeader>
                      <CardContent>
