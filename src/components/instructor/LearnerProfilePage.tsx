...
export function LearnerProfilePage({ learnerId, learnerName, onBack, onRemove }: LearnerProfilePageProps) {
  ...
  const loadLearnerProfile = async () => {
    try {
      ...
      if (!user) {
        ...
        console.log('Available user IDs:', users.map((u: any) => u.id));
      ...
      const userEnrollments = allEnrollments.filter((e: any) => e.user_id === learnerId);
      ...
      const enrollments: CourseEnrollment[] = userEnrollments.map((enrollment: any) => {
        const course = courses.find((c: any) => c.id === enrollment.course_id);
        const performance = 
          enrollment.progress >= 80 ? 'Excellent' :
          enrollment.progress >= 50 ? 'Good' : 'Needs Help';
        
        const enrolledDate = new Date(enrollment.enrolled_at);
        const daysSince = Math.floor((Date.now() - enrolledDate.getTime()) / (1000 * 60 * 60 * 24));
        const lastAccessed = daysSince === 0 ? 'Today' : 
      ...
      const totalTime = enrollments.reduce((sum, e) => sum + (e.progress * 1.2), 0);
      ...
      setProfile({
        ...
        email: user.email,
        avatar: user.avatar,
        joinedAt: enrollments.length > 0 
          ? new Date(Math.min(...enrollments.map(e => new Date(e.enrolledAt).getTime()))).toISOString()
  ...
  const handleRemoveLearner = () => {
    ...
    if (confirm(`Are you sure you want to remove ${profile.name} from all courses? This action cannot be undone.`)) {
      if (onRemove) {
        onRemove(learnerId);
      }
      onBack();
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#fafbff] via-[rgba(240,249,255,0.3)] to-[#fafbff] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg font-medium text-muted-foreground">Learner not found</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-16 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h1 className="text-4xl font-extrabold text-slate-900">{profile.name}</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{profile.averageProgress}%</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{profile.totalLearningTime}h</div>
              <p className="text-xs text-muted-foreground mt-1">
                Total hours
              </p>
            </CardContent>
                ...
                {profile.enrollments.map((enrollment) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <h3 className="font-semibold text-slate-900">{enrollment.courseName}</h3>
                      <div className="flex items-center gap-3 mt-1">
                        <span className="text-sm text-slate-500">{enrollment.courseCategory}</span>
                        <span className="text-slate-300">•</span>
                        <span className="text-sm text-slate-500">Last accessed: {enrollment.lastAccessed}</span>
                      </div>
                    </div>
                    ...
                    <div className="text-right min-w-[200px]">
                      <div className="flex items-center justify-end gap-3 mb-2">
                        ...
                        <Badge 
                          variant={
                            enrollment.performance === 'Excellent' ? 'default' :
                            enrollment.performance === 'Good' ? 'secondary' :
                            'destructive'
                          }
                        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>How the learner is performing across courses</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {[
                  { 
                    label: 'Excellent', 
                    count: profile.enrollments.filter(e => e.performance === 'Excellent').length,
                    color: 'bg-primary',
                    textColor: 'text-primary'
                  },
                  { 
                    label: 'Good', 
                    count: profile.enrollments.filter(e => e.performance === 'Good').length,
                    color: 'bg-secondary',
                    textColor: 'text-secondary'
                  },
                  { 
                    label: 'Needs Help', 
                    count: profile.enrollments.filter(e => e.performance === 'Needs Help').length,
                    color: 'bg-destructive',
                    textColor: 'text-destructive'
                  }
                ].map((item) => {
                  const percentage = profile.totalCourses > 0 
                    ...
                    <div key={item.label}>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`text-sm font-bold ${item.textColor}`}>
                          {item.count} course{item.count !== 1 ? 's' : ''} ({Math.round(percentage)}%)
                        </span>
                      </div>
                ...
                {profile.enrollments.slice(0, 5).map((enrollment) => (
                  <div key={enrollment.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-500">Last accessed {enrollment.lastAccessed}</p>
                    </div>
