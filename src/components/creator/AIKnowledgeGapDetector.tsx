...
export function AIKnowledgeGapDetector({
  courseData,
  onApplySuggestion,
}: AIKnowledgeGapDetectorProps) {
  ...
  const analyzeKnowledgeGaps = () => {
    // Only show the spinner on the very first analysis.
    // Subsequent silent re-analyses avoid the layout shift that causes scrolling.
    ...
    setTimeout(() => {
      ...
      if (!courseData.prerequisites || courseData.prerequisites.length === 0) {
        detectedGaps.push({
          ...
          suggestion:
            'Add prerequisites to help students understand what knowledge they need before starting',
        });
      }
      ...
      if (
        courseData.sections &&
        courseData.sections.some((s: any) => s.lessons && s.lessons.length > 8)
      ) {
        detectedGaps.push({
          id: 'gap-2',
          ...
          suggestion:
            'Consider breaking large modules into smaller, focused units for better learning outcomes',
        });
      }

      // Check for weak transitions
      if (courseData.sections && courseData.sections.length > 1) {
        const sectionsWithoutDesc = courseData.sections.filter(
          (s: any) => !s.description || !s.outcome,
        );
        if (sectionsWithoutDesc.length > 0) {
          detectedGaps.push({
            ...
            suggestion:
              'Add module descriptions and learning outcomes to improve flow and student understanding',
          });
        }
      }
      ...
      if (!courseData.assessments || courseData.assessments.length === 0) {
        detectedGaps.push({
          ...
          suggestion:
            'Add quizzes and exercises to reinforce learning and track student progress',
        });
      }
        ...
        <CardContent className='p-6'>
          <div className='flex items-center gap-3'>
            ...
            <p className='text-sm text-amber-700'>
              AI is analyzing your course structure...
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <CardHeader>
        <div className='flex items-center justify-between'>
          ...
          <Badge variant='secondary'>
            {visibleGaps.length}{' '}
            {visibleGaps.length === 1 ? 'suggestion' : 'suggestions'}
          </Badge>
        </div>
        <CardDescription>
