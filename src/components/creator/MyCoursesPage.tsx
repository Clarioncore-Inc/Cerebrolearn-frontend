...
export function MyCoursesPage({ onNavigate, onCreateCourse }: MyCoursesPageProps) {
  ...
  const [selectedCourseForShare, setSelectedCourseForShare] = useState<any>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Button size="lg" className="bg-primary hover:bg-primary/90 transition-colors" onClick={onCreateCourse}>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="w-full md:w-auto">
                  ...
                  Sort: {sortBy === 'recent' ? 'Recent' : sortBy === 'popular' ? 'Popular' : 'Revenue'}
                </Button>
              </DropdownMenuTrigger>
        ...
        <TabsContent value={filterStatus} className="mt-6">
                ...
                >
                  ...
                  <div className="relative h-[200px] overflow-clip">
                    ...
                    <div className="absolute top-3 left-3">
                      ...
                      >
                        <CheckCircle2 className={`w-3 h-3 mr-1 ${course.status === 'published' ? 'block' : 'hidden'}`} />
                        <Clock className={`w-3 h-3 mr-1 ${course.status === 'draft' ? 'block' : 'hidden'}`} />
                    ...
                    <div className="absolute top-3 right-3 flex flex-col gap-1.5">
                      <div className="backdrop-blur-md bg-white/95 rounded-lg px-2 py-1 flex items-center gap-1 border border-white/50 shadow-md">
                        ...
                        <span className="text-xs font-bold text-gray-900">{course.enrollments || 0}</span>
                      </div>
                      {course.rating > 0 && (
                        <div className="backdrop-blur-md bg-white/95 rounded-lg px-2 py-1 flex items-center gap-1 border border-white/50 shadow-md">
                  ...
                  <div className="pt-[0px] pr-[16px] pb-[16px] pl-[16px]">
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <Badge variant="outline" className="border-primary/30 text-primary bg-primary/5 font-semibold px-2.5 py-0.5">
                        {course.category}
                      </Badge>
                    ...
                    <div className="flex items-center flex-wrap gap-3 text-xs text-muted-foreground mb-3 pb-3 border-b border-gray-200">
                      <div className="flex items-center gap-1 bg-gray-50 rounded-md px-2 py-1">
                        ...
                        <span className="font-medium">{course.lessons || 0} lessons</span>
                      </div>
                      <div className="flex items-center gap-1 bg-gray-50 rounded-md px-2 py-1">
                        <Clock className="w-3.5 h-3.5 text-primary" />
                        <span className="font-medium">{course.duration || '0h'}</span>
                      </div>
                      {course.reviews > 0 && (
                        <div className="flex items-center gap-1 bg-gray-50 rounded-md px-2 py-1">
                    ...
                    {course.status === 'draft' && (
                      <div className="mb-3 bg-amber-50 rounded-lg p-3 border border-amber-200">
                        <div className="flex items-center justify-between text-xs mb-1.5">
                          <span className="font-medium text-amber-700">Course Completion</span>
                          <span className="font-bold text-amber-900">{course.completion || 0}%</span>
                        </div>
                        <Progress value={course.completion || 0} className="h-2 bg-amber-100" />
                      </div>
                    )}
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-xl font-bold text-emerald-700">${(course.revenue || 0).toLocaleString()}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                          ...
                          <div className="flex items-center gap-2 ml-4">
                            ...
                            {course.status === 'draft' ? (
                              ...
                              <Button
                                size="sm"
                                variant="secondary"
                                onClick={() => onNavigate('creator-analytics', course)}
                              >
          ...
          {filteredCourses.length === 0 && (
            <Card className="p-16 text-center border-2 border-dashed">
              ...
              <Button size="lg" onClick={onCreateCourse} className="bg-primary hover:bg-primary/90 transition-colors">
                <Plus className="mr-2 h-5 w-5" />
      ...
      <Dialog open={shareDialogOpen} onOpenChange={setShareDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Share &quot;{selectedCourseForShare?.title}&quot; with your audience
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            {/* Copy Link Section */}
            <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
              <label className="text-sm font-medium mb-2 block">Course Link</label>
              <div className="flex items-center gap-2">
                <Input
                  type="text"
                  value={selectedCourseForShare ? `https://cerebrolearn.com/course/${selectedCourseForShare.id}` : ''}
                  readOnly
                ...
                <Button
                  size="sm"
                  onClick={() => {
                    navigator.clipboard.writeText(`https://cerebrolearn.com/course/${selectedCourseForShare?.id}`);
            ...
            {selectedCourseForShare && (
              <div className="p-4 bg-gradient-to-br from-primary/5 to-secondary/5 rounded-lg border border-primary/20">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold mb-1 truncate">{selectedCourseForShare.title}</h4>
