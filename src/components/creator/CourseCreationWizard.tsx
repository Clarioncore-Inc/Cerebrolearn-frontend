...
import { BookOpen, Video, FileText, Code, CheckCircle, DollarSign, Settings, Eye, Upload, X, Plus, GripVertical, Image as ImageIcon } from 'lucide-react';
...
export function CourseCreationWizard({ onComplete, onCancel, initialData }: CourseCreationWizardProps) {
  ...
  const [courseData, setCourseData] = useState({
    ...
    modules: initialData?.modules || [ // New: Enhanced module structure
      {
        id: '1',
        title: '',
        description: '',
        outcome: '',
        lessons: [
          { id: '1', title: '', type: 'video', duration: '', content: '', order: 1 }
        ],
        order: 1
      }
    ],
  ...
  const validateStep = (step: number) => {
    switch (step) {
      case 1:
        ...
        return true;
      case 2:
        const hasLessons = courseData.sections.some(s => s.lessons.length > 0);
        if (!hasLessons) {
      ...
      sections: prev.sections.map(section =>
        section.id === sectionId
          ? {
              ...
              lessons: [
                ...section.lessons,
                { id: Date.now().toString(), title: '', type: 'video', duration: '', content: '' }
              ]
            }
          : section
      )
  ...
  const handlePublish = async () => {
    ...
    if (courseData.learningObjectives.filter(obj => obj.trim()).length === 0) {
      toast.error('At least one learning objective is required');
      return;
    }
    if (courseData.sections.filter(s => s.title && s.lessons.length > 0).length === 0) {
    ...
    try {
      ...
      const course = {
        ...
        sections: courseData.sections.filter(s => s.title && s.lessons.length > 0),
        ...
        price: courseData.priceType === 'paid' ? parseFloat(courseData.price) : 0,
        currency: courseData.currency,
        discountPrice: courseData.discountPrice ? parseFloat(courseData.discountPrice) : null,
        ...
        maxStudents: courseData.maxStudents ? parseInt(courseData.maxStudents) : null,
        status: 'published',
        public: true,
        instructorId: user?.id || 'demo-instructor',
        instructorName: user?.user_metadata?.name || user?.email || 'Demo Instructor',
      ...
      try {
        const existingCourses = JSON.parse(localStorage.getItem('cerebrolearn_courses') || '[]');
        existingCourses.push(course);
        localStorage.setItem('cerebrolearn_courses', JSON.stringify(existingCourses));
  ...
  const handleSaveAsDraft = async () => {
    ...
    try {
      ...
      const course = {
        ...
        targetAudience: courseData.targetAudience,
        priceType: courseData.priceType,
        price: courseData.priceType === 'paid' && courseData.price ? parseFloat(courseData.price) : 0,
        currency: courseData.currency,
        discountPrice: courseData.discountPrice ? parseFloat(courseData.discountPrice) : null,
        allowReviews: courseData.allowReviews,
        enableCertificate: courseData.enableCertificate,
        ...
        instructorName: user?.user_metadata?.name || user?.email,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
        enrollments: 0,
        rating: 0,
      ...
      // Use dummy data mode - save draft to localStorage
      try {
        const existingDrafts = JSON.parse(localStorage.getItem('cerebrolearn_drafts') || '[]');
        existingDrafts.push(course);
        localStorage.setItem('cerebrolearn_drafts', JSON.stringify(existingDrafts));
    ...
    <div className="w-full">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8 flex items-start justify-between">
          <div>
            <h1 className="text-4xl font-extrabold text-gray-900 mb-2">Create New Course</h1>
            ...
            {steps.map((step, index) => (
              <React.Fragment key={step.id}>
                <div className="flex flex-col items-center flex-1">
                  ...
                  <span className="text-sm font-medium text-gray-700 text-center">{step.name}</span>
                </div>
                {index < steps.length - 1 && (
                  <div className={`flex-1 h-1 mx-4 rounded ${currentStep > step.id ? 'bg-green-500' : 'bg-gray-200'}`} />
                )}
              </React.Fragment>
            ))}
          </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
          ...
          {currentStep === 1 && (
            <div className="space-y-6">
              ...
              {courseData.category && courseData.level && (
                <AIAssistPanel
                  type="title"
                  context={{ category: courseData.category, level: courseData.level }}
                  onAccept={(suggestion) => {
                    setCourseData(prev => ({ ...prev, title: suggestion }));
                    toast.success('Title suggestion applied! You can edit it anytime.');
                  }}
              ...
              {courseData.category && courseData.level && (
                <AIAssistPanel
                  ...
                  context={{ category: courseData.category, level: courseData.level }}
                  onAccept={(suggestion) => {
                    setCourseData(prev => ({ ...prev, description: suggestion }));
                    toast.success('Description suggestion applied! You can edit it anytime.');
                  }}
                  onReject={() => toast.info('Suggestion dismissed')}
                />
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setCourseData(prev => ({ ...prev, category: value, subcategory: '' }))}
                  >
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setCourseData(prev => ({ ...prev, subcategory: value }))}
                  ...
                  >
                    ...
                    <SelectContent>
                      {categories.find(cat => cat.name === courseData.category)?.subcategories.map(subcategory => (
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-[8px] p-8 text-center hover:border-[#395192] transition-colors">
                  ...
                  <label htmlFor="thumbnail-upload" className="cursor-pointer">
                    <p className="text-gray-700 mb-1">
                      <span className="text-[#395192] hover:underline">Click to upload</span> or drag and drop
                    </p>
                    <p className="text-sm text-gray-500">PNG, JPG (recommended: 1280x720)</p>
                  </label>
                  {courseData.thumbnail && (
                    <p className="mt-2 text-sm text-green-600">✓ {courseData.thumbnail.name}</p>
                  )}
                </div>
              </div>
            </div>
                  ...
                  <div key={index} className="flex gap-2 mb-2">
                    <Input
                      ...
                      onChange={(e) => {
                        setCourseData(prev => ({
                          ...prev,
                          learningObjectives: prev.learningObjectives.map((o, i) =>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-gray-600">Structure your course into sections and lessons</p>
                </div>
                <button
                  onClick={addSection}
              ...
              {courseData.sections.length === 0 && courseData.title && courseData.category && (
                <AIAssistPanel
                  type="structure"
                  context={{ title: courseData.title, category: courseData.category, level: courseData.level }}
                  onAccept={(suggestion) => {
                    const generatedSections = suggestion.modules.map((mod: any, index: number) => ({
                      id: Date.now().toString() + index,
                      title: mod.title,
                      description: mod.outcome,
                      lessons: mod.lessons.map((lessonTitle: string, lIndex: number) => ({
                    ...
                    setCourseData(prev => ({ ...prev, sections: generatedSections }));
                    toast.success('Course structure generated! You can edit any section.');
                  }}
                />
              )}
              ...
              {courseData.sections.length > 0 && (
                <AIKnowledgeGapDetector
                  ...
                  onApplySuggestion={(gapId, suggestion) => {
                    toast.success('Suggestion noted! You can implement it as needed.');
                  }}
                />
              )}

              {courseData.sections.map((section, sectionIndex) => (
                <div key={section.id} className="border border-gray-200 rounded-[8px] p-6">
                  ...
                  <div className="ml-9 space-y-3">
                    {section.lessons.map((lesson, lessonIndex) => (
                      <div key={lesson.id} className="flex items-center gap-3 bg-gray-50 p-3 rounded-[8px]">
          ...
          {currentStep === 4 && (
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    onClick={() => setCourseData(prev => ({ ...prev, priceType: 'free' }))}
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-600">Anyone can enroll for free</p>
                  </button>
                  <button
                    onClick={() => setCourseData(prev => ({ ...prev, priceType: 'paid' }))}
                    className={`p-4 border-2 rounded-[8px] text-left transition-all ${
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-600">Students must pay to enroll</p>
                  </button>
                </div>
              </div>
                ...
                <>
                  ...
                  {courseData.price && (
                    <div className="bg-green-50 border border-green-200 rounded-[8px] p-4">
                      <p className="text-sm text-green-800 mb-2">Pricing Preview</p>
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-2xl font-bold text-gray-900">
                          {courseData.currency} {courseData.discountPrice || courseData.price}
                        </span>
                        {courseData.discountPrice && (
                          <span className="px-2 py-1 bg-red-100 text-red-700 text-sm font-medium rounded">
                            {Math.round((1 - parseFloat(courseData.discountPrice) / parseFloat(courseData.price)) * 100)}% OFF
                          </span>
                        )}
                      </div>
                    </div>
          ...
          {currentStep === 5 && (
            <div className="space-y-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">Publish Settings</h2>
              ...
              <div className="space-y-4">
                <label className="flex items-center justify-between p-4 border border-gray-200 rounded-[8px] cursor-pointer hover:bg-gray-50">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">Allow students to leave reviews</p>
                  </div>
                  <input
                    type="checkbox"
                    checked={courseData.allowReviews}
                    onChange={(e) => setCourseData(prev => ({ ...prev, allowReviews: e.target.checked }))}
                ...
                <label className="flex items-center justify-between p-4 border border-gray-200 rounded-[8px] cursor-pointer hover:bg-gray-50">
                  <div>
                    <p className="font-medium text-gray-900">Enable Certificate</p>
                    <p className="text-sm text-gray-600">Award certificates upon completion</p>
                  </div>
                  <input
                    type="checkbox"
                    checked={courseData.enableCertificate}
                    onChange={(e) => setCourseData(prev => ({ ...prev, enableCertificate: e.target.checked }))}
                    className="w-5 h-5 text-[#395192] rounded"
                  />
                </label>
                ...
                <label className="flex items-center justify-between p-4 border border-gray-200 rounded-[8px] cursor-pointer hover:bg-gray-50">
                  <div>
                    <p className="font-medium text-gray-900">Enable Discussions</p>
                    <p className="text-sm text-gray-600">Allow course discussions and Q&A</p>
                  </div>
                  <input
                    type="checkbox"
                    checked={courseData.enableDiscussions}
                    onChange={(e) => setCourseData(prev => ({ ...prev, enableDiscussions: e.target.checked }))}
                    className="w-5 h-5 text-[#395192] rounded"
                  />
                </label>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-[8px] p-6">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="text-blue-900 font-medium">{courseData.title || 'Not set'}</p>
                  </div>
                  <div>
                    <p className="text-blue-600 mb-1">Category</p>
                    <p className="text-blue-900 font-medium">{courseData.category || 'Not set'}</p>
                  </div>
                  <div>
                    <p className="text-blue-600 mb-1">Level</p>
                    <p className="text-blue-900 font-medium">{courseData.level || 'Not set'}</p>
                  </div>
                  <div>
                    <p className="text-blue-600 mb-1">Sections</p>
                    <p className="text-blue-900 font-medium">{courseData.sections.length}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-blue-900 font-medium">
                      {courseData.priceType === 'free' ? 'Free' : `${courseData.currency} ${courseData.price}`}
                    </p>
                  </div>
                </div>
              </div>
